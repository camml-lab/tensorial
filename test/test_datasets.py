...
def test_generate_batches_batch_builder():
    ...
    batches = tuple(datasets.generate_batches(batch_size, inputs, outputs, output_batch_builder=np.array))
    assert isinstance(batches[0].inputs, tuple)
    assert isinstance(batches[0].targets, np.ndarray)

    # Specify both separately
    batches = tuple(
        datasets.generate_batches(batch_size, inputs, outputs, batch_builder=list, output_batch_builder=np.array)
    )
