...
def test_graph_metric(cube_graph: jraph.GraphsTuple, rng_key):
    ...
    # This time use kwargs to graph_metric which will be passed to Std by kwargs this time (instead of args)
    ...
    assert jnp.isclose(res.compute(), optax.squared_error(nodes['size_predictions'], nodes['sizes']).mean())
