...
def generated_padded_graphs(dataset: GraphDataset,
                            add_mask=False,
                            num_nodes=None,
                            num_edges=None,
                            num_graphs=None) -> Iterator[GraphBatch]:
    """Provides an iterator over graphs tuple batches that are padded to make the number of nodes, edges and graphs in
    each batch equal to the maximum found in the dataset"""
...
def add_padding_mask(
    ...
    """
    Add a mask array to the `mask_field` of `graph` for either nodes, edges and/or globals which can be used to
    determine which entries are there just for padding (and therefore should be ignored in any computations).

    If `overwrite` is `True` then any mask already found in the mask field will be overwritten by the padding mask.
    Otherwise, it will be ORed.
    """
...
def generate_batches(
    ...
    return datasets.generate_batches(batch_size, inputs, outputs, batch_builder, output_batch_builder)
