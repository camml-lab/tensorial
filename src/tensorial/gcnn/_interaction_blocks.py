...
class InteractionBlock(linen.Module):
    ...
    def __call__(
        ...
        output_irreps = e3j.Irreps(self.irreps_out).regroup()  # The irreps to use for the output node features
        ...
        edge_features = e3j.tensor_product(messages, edge_features, filter_ir_out=output_irreps + '0e')

        # Make a compound message
        messages = e3j.concatenate([messages.filter(output_irreps + '0e'), edge_features]).regroup()

        # Now, based on the messages irreps, create the radial MLP that maps from inter-atomic distances to tensor
        # product weights
